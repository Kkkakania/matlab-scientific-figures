...
for k = 1:numel(registry)
    ...
    seed = sftExampleDataSeed(registry(k).SyntheticDataKind);
    manifest(k).SyntheticDataKind = seed.Kind;
    manifest(k).SyntheticDataSeed = seed.Seed;
    manifest(k).SyntheticDataRng = seed.RngAlgorithm;
