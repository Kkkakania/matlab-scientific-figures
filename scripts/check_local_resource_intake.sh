...
grep -q "239 template ideas" "$DOC"
grep -q "60 palette families" "$DOC"
grep -q "Go-side helper experiments" "$DOC"
grep -q "color-science audit" "$DOC"
...
grep -q "power and energy plots" "$BACKLOG"
grep -q "diagram-style figures" "$BACKLOG"
