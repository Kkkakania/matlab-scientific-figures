...
grep -q "Next Candidate" "$DOC"
grep -q "transformer-loading-heatmap" "$DOC"
grep -q "Do not add a fourth electrical figure" "$DOC"
grep -q "docs/domain-examples.md" "$README"
grep -q "docs/domain-examples.md" "$EXAMPLES_README"
grep -q "three-phase" "$README"
grep -q "renderThreePhaseWaveform" "$EXAMPLES_README"
